...
```bash
scripts/install.sh --version 1.0.0 --prefix "$HOME/.local" \
  --base-url https://github.com/OriginalFunction/Conch/releases/download/v1.0.0
```

**Homebrew** (formula checksums come from release automation, not hand edits):

```bash
scripts/install-homebrew.sh --version 1.0.0
```
...
```bash
sudo -E scripts/install-debian.sh --version 1.0.0
```
